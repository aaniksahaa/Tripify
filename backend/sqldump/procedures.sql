...
CREATE OR REPLACE PROCEDURE DELETE_HOTEL(HID IN NUMBER) IS
...
BEGIN 
	...
	LOOP 
		...
		VALUES(K.USER_ID, 'A hotel named - ' || K.HOTEL_NAME || ' which you booked for the trip - ' || K.TRIP_NAME || ' was deleted!');
...
CREATE OR REPLACE PROCEDURE DELETE_RESTAURANT(RID IN NUMBER) IS
...
BEGIN 
	...
	LOOP 
		...
		INSERT INTO NOTIFICATIONS(USER_ID,TEXT)
		VALUES(K.USER_ID, 'A restaurant named - ' || K.RESTAURANT_NAME || ' which you booked for the trip - ' || K.TRIP_NAME || ' was deleted!');
